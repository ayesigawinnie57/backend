...
import { adminFlashSalesApi, adminProductsApi, type ApiFlashSaleItem, type ApiProduct } from '../../lib/api'
...
export default function AdminFlashSales() {
  ...
  useEffect(() => {
    Promise.all([
      adminFlashSalesApi.list().then(({ data }) => setSales(Array.isArray(data) ? data : (data as any).results ?? [])),
      adminProductsApi.listAll().then(setProducts),
    ])
      .catch(err => setLoadError(parseError(err, 'Failed to load flash sales.')))
  ...
  const handleSave = async () => {
    ...
    setError('')
    try {
      const { data } = await adminFlashSalesApi.update(editing.id, {
      ...
      setError(parseError(e, 'Failed to save.'))
    } finally {
      setSaving(false)
    }
  }
  ...
  const handleDelete = async (s: ApiFlashSaleItem) => {
    ...
    try {
      ...
      setLoadError(parseError(err, 'Failed to delete flash sale.'))
    }
  }

  const usedProductIds = new Set(
    sales.filter(s => !s.is_expired && s.is_active && (!editing || s.id !== editing.id)).map(s => String(s.product.id))
  )
    ...
    <div className="p-8">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-xl font-extrabold text-[#071A2B]">Flash Sales</h1>
        </div>
        <button
          onClick={() => navigate('/admin/flashsales/add')}
        ...
        <div className="fixed inset-0 bg-black/50 flex items-end justify-center z-50">
          <div className="bg-white rounded-t-2xl p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-[17px] font-extrabold text-[#071A2B]">Edit Flash Sale</h2>
              <button onClick={() => setModal(false)}><X size={20} color="#071A2B" /></button>
            </div>
                    ...
                    <div className="max-h-40 overflow-y-auto">
                      {filteredProducts.map(p => (
                        <button
                          ...
                          className="flex items-center gap-3 w-full py-2.5 border-b border-[#F1F5F9] hover:bg-[#F8FAFC] text-left"
                        >
